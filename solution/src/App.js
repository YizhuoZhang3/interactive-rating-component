  ...
  const Button = ({number}) => {
    return (
      <button 
        ...
        className="btn-number bg-gray-700 h-10 w-10 rounded-full text-gray-200 focus:bg-gray-400 transition-all duration-150">{number}
      </button>
    )
  }
  return (
    ...
    {!isSubmitted && (
      ...
      <div className="text-center">
        <button 
          ...
          className="btn-rating w-full text-white uppercase tracking-wide pt-3 pb-2 rounded-full">Submit</button>
      </div>
    </div>
    )}
...
const ThankYou = ({setIsSubmitted, items}) => {
  return (
    <div className="wrapper">
      <img src={thanks} className="block mx-auto mb-5" alt="thanks"/>
      <div className="flex items-center justify-center">
        <p className="p-rating bg-gray-700 rounded-full text-center text-sm px-4">You selected {items} out of 5</p>
      </div>
  ...
  don’t hesitate to get in touch!</p>
      <div className="text-center">
        <button 
          ...
          className="btn-rating w-full text-white uppercase tracking-wide pt-3 pb-2 rounded-full">Rate Again</button>
      </div>
    </div>
  )
}
